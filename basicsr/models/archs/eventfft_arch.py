...
# encoder加入原来的attention，暴力做
class eventfft_directfuse(nn.Module):
    def __init__(self,
                 evt_inp=6,
                 img_inp=3,
                 out_channels=3,
        ...
        super(eventfft_directfuse, self).__init__()
        ...
        self.encoder_level1 = nn.Sequential(*[
            TransformerBlock(dim=dim, ffn_expansion_factor=ffn_expansion_factor, bias=bias, att=True) for i in
            range(num_blocks[0])])
    ...
    def forward(self, x, event):
        # 输入
        evt_inp = self.norm1(self.evt_embed(event))
        img_inp = self.norm2(self.img_embed(x))
        
        inp_enc_level1 = self.conv1(torch.cat((img_inp, evt_inp), dim=1))
...
# 原文设计，只是把输入cat一个event
class eventfft_concate(nn.Module):
    """
    FFTformer 主体：三层编码 + 两层解码 + refinement
    - encoder: L1(原分辨率)->down->L2->down->L3
    - decoder: L3(att)->up+fuse(L2 skip)->L2(att)->up+fuse(L1 skip)->L1(att)
    - refinement(att) 后输出 Conv，并与输入做全局残差相加
    """
    def __init__(self,
                 inp_channels=9,
                 out_channels=3,
        ...
        super(eventfft_concate, self).__init__()
    ...
    def forward(self, x, event):
        # 输入: (B,3,H,W)
        inp_img = torch.cat((x, event), dim=1)  # (B,9,H,W)
