...
class ImageEventRestorationModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        # set up optimizers and schedulers，scheduler在base_model里
    ...
    def optimize_parameters(self, current_iter): # train one step
        ...
        l_total.backward()
        accu = (current_iter % accumulation_steps == 0) 
        if accu:
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if save_img:    
                if self.opt['is_train']:
                    if cnt % 3 == 0: # 每3个图像保存一个
                        print('Save path:{}'.format(self.opt['path']['visualization']))
                        print('Dataset name:{}'.format(dataset_name))
                        print('Img_name:{}'.format(f'{current_iter:07d}_{img_name}.png'))
                        save_img_path = osp.join(self.opt['path']['visualization'], dataset_name,
                                                f'{current_iter:07d}_{img_name}.png')
                        
                        save_gt_img_path = osp.join(self.opt['path']['visualization'], dataset_name,
                                                f'{current_iter:07d}_{img_name}_gt.png')
                        imwrite(sr_img, save_img_path)
                        imwrite(gt_img, save_gt_img_path)
                else:
                    print('currently only support save validation images during training.')
                    print('if for testing, use Test_image_event_restoration_model')
